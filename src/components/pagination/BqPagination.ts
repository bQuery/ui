...
type BqPaginationProps = { page: number; total: number; 'sibling-count': number; disabled: boolean };

function buildPages(page: number, total: number, siblings: number): Array<number | '...'> {
  if (total <= 7 + siblings * 2) return Array.from({ length: total }, (_, i) => i + 1);
...
const definition: ComponentDefinition<BqPaginationProps> = {
  ...
  connected() {
    ...
    const handler = (e: Event) => {
      const btn = (e.target as Element).closest('.page-btn') as HTMLElement | null;
      if (!btn || btn.hasAttribute('disabled') || self.hasAttribute('disabled')) return;
      const p = parseInt(btn.getAttribute('data-page') ?? '0', 10);
      if (!p || isNaN(p)) return;
      self.setAttribute('page', String(p));
      self.dispatchEvent(new CustomEvent('bq-page-change', { detail: { page: p }, bubbles: true, composed: true }));
    };
  ...
  render({ props }) {
    const page = props.page; const total = props.total; const siblings = props['sibling-count'];
    ...
    return html`
      <nav aria-label="${t('pagination.nav')}" part="nav">
        <div class="pagination" part="pagination">
          <button class="page-btn" data-page="${String(page - 1)}" ${prevDisabled ? 'disabled' : ''} aria-label="${t('pagination.prev')}" type="button">&#8249;</button>
          ${pageItems}
          <button class="page-btn" data-page="${String(page + 1)}" ${nextDisabled ? 'disabled' : ''} aria-label="${t('pagination.next')}" type="button">&#8250;</button>
        </div>
      </nav>
    `;
  },
