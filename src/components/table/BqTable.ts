...
const definition: ComponentDefinition<BqTableProps> = {
  ...
  render({ props }) {
    ...
    const theads = cols.map(col => {
      ...
      return `<th part="th" ${col.sortable ? `class="sortable" data-sort-key="${escapeHtml(col.key)}" tabindex="0" role="columnheader" aria-sort="${isSorted ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none'}"` : ''}>${escapeHtml(col.label)}${sortIcon}</th>`;
    }).join('');
